/* Să se scrie un program care ţine evidenţa mașinilor de spălat dintr-un magazin. Pentru fiecare produs se specifică în parte: producător, model, pret de furnizor, preț de vânzare,
 greutate, tip încărcare (frontală/verticală). În functie de tipul de încărcare introdus de la tastatură se vor citi următoarele caracteristici:
...
4. Crearea unui fișier numit „oferta.txt” și salvarea tuturor mașinilor de spălat care au un preț mai mic dat de la tastatură.Nota: In cadrul fisierului oferta.txt informatia va fi 
structurata astfel incat fiecare linie din cadrul fisierului oferta.txt sa contina informatia completa despre o masina de spalat. Informatiile vor fi separate prin spatii astfel:
